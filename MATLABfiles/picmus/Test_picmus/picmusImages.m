...
DENOpath = [pwd, '/picmus/Test_picmus/results/DENO/'];
DRUSpath = [pwd, '/picmus/Test_picmus/results/DRUS/'];

repeat = 10;
phantoms = {'simu_reso', 'simu_cont', 'expe_reso', 'expe_cont', 'expe_cross', 'expe_long'};
numPhans = length(phantoms);
...
for phanId = phanSelected 
    rowId = rowId + 1;
        ...
        for c = 1:repeat
            load([DENOpath, num2str(phanId + (c-1)*numPhans), '_-1.mat']);
            temp(c,:,:) = squeeze(x);
        end
        ...
        temp = zeros(repeat, 256, 256);
        for c = 1:repeat
            load([DRUSpath, num2str(phanId + (c-1)*numPhans), '_-1.mat']);
            temp(c,:,:) = squeeze(x);
        end

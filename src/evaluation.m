function [FWHMA,FWHML, CNR, gCNR, SNR, KS] = evaluation(path_phantom,image,flagDisplay,flagAvg)
...
%         CNR = CNRres;
        ...
        CNR = CNR.score; %--all values in a vector
        if flagAvg 
            CNR = mean(CNR.score(:));
        end
...
%         gCNR = gCNRres;
        ...
        gCNR = gCNR.score;  %--all values in a vector
        if flagAvg
...
%         end
%         KS = KSres;
        KS = KS.score; %--all values in a vector
        if flagAvg
            KS = mean(KS.score(:));
        end
...
%         SNR = SNRres;  
        ...
        SNR = SNR.score; %--all values in a vector
        if flagAvg
            SNR = mean(SNR.score(:));
        end
    end
